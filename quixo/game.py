...
class Game(object):
    ...
    def possible_moves (self,player_id:int):
        ...
        for row in row_perimeter:
            for col in range(0,5):
                if self._board[row,col] == -1 or self._board[row,col] == player_id:
        ...
        for col in col_perimeter:
            for row in range(0,5):
                if self._board[row,col] == -1 or self._board[row,col] == player_id:
                    if (row,col) not in temp_possible_moves:
                        temp_possible_moves.append(((row,col),Move.BOTTOM))
                        temp_possible_moves.append(((row,col),Move.LEFT))
                        temp_possible_moves.append(((row,col),Move.TOP))
                        temp_possible_moves.append(((row,col),Move.RIGHT))
    ...
    def get_new_state(self, from_pos: tuple[int, int], slide: Move, player_id: int):
        new_game = deepcopy(self)
        prev_value = deepcopy(new_game._board[(from_pos[1], from_pos[0])])
        acceptable = new_game.__take((from_pos[1], from_pos[0]), player_id)
        if acceptable:
            acceptable = new_game.__slide(from_pos, slide)
            if not acceptable:
                new_game._board[(from_pos[1], from_pos[0])] = deepcopy(prev_value)
        return new_game
